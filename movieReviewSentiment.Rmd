...
第三種作法使用 Word2Vec，利用前面23908個詞彙產生term-co-occurence matrix (TCM)、建立 word vectors (size 設定為500)
...
Word2Vec 預測得到的 Confusion Matrix 如下，Accuracy : 0.8364 (比前2次還低)
...
相同的測試句，Word2Vec model的預測結果為 0: negative (預測正確!)
...
* #### __Word2Vec model including unlabeledTrainData __

雖然 Word2Vec 目前的表現在上述三種中是最差的，不過它使用的維度只有500，和前2種作法(維度:23908)相比明顯低非常的多，加上目前都只使用了 labeledTrainData.tsv 進行訓練(25000筆 * 0.7)，另外還有 unlabeledTrainData.tsv (50000筆)尚未利用。
...
這次 Word2Vec 預測得到的 Confusion Matrix 如下，Accuracy : 0.8513 (明顯比前次 0.8364 高)
...
上述都使用邏輯迴歸的方法，從ROC curve 黃色線可以看出 Word2Vec model 表現已經很不錯了，再者，它使用的維度只有500。因此，接下來利用不同的演算法於 Word2Vec model 上試試，以下是其他演算法的表現
...
使用 randomForest 套件 randomForest (ntree = 500, mtry = 500)
...
從以上發現 SVM + Word2Vec model 的 Accuracy 是目前最高的，接下來利用 Cost 參數來優化 model。下圖是不同 Cost 參數執行的結果，可看出 C=3 時可以得到最佳的 Accuracy (0.8636)
...
試著將以上執行結果彙整(如下表)，可以發現傳統向量空間模型(Bag of words、Tf-idf)的表現有一定水準，不過必須透過 labeled 資料進行訓練、建模，另外，高維度也是不可避免的，高稀疏值雖可設定刪除，但也會影響準確率。Word2Vec 方法則可利用 labeled 和 unlabeled 資料生成參照的 word vectors，參照資料越多、準確率也可望提升(參考 ROC curve 綠、黃、黑)，另外，維度的設定也可兼顧運算成本與效能。這次練習設定的維度為500，嘗試3種不同的演算法，發現 Word2Vec + SVM 的表現不俗(比一開始的logistic regression還好)，最後試著調整 Cost 參數，得到 C=3 時準確率最佳

```{r, echo=FALSE}
model <- c('Bag of words','Tf-idf','Word2Vec','Word2Vec','Word2Vec','Word2Vec','Word2Vec','Word2Vec')

...
雖然 Word2Vec 目前的表現在上述三種中是最差的，不過它使用的維度(500)在運算效能上有優勢，加上目前都只使用了 labeledTrainData.tsv 進行訓練(25000筆 * 0.7)，另外還有 unlabeledTrainData.tsv (50000筆)尚未利用。
...
上述都使用邏輯迴歸的方法，從ROC curve 黃色線可以看出 Word2Vec model 表現已經很不錯了。因此，接下來利用不同的演算法於 Word2Vec model 上試試，以下是其他演算法的表現
